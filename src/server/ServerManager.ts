...
// Sub-agent support: the v2 engine has no task tool (nor plugin/MCP tools),
// so the model delegates through the `oc-task` skill, which teaches a marked
// bash command running out/oc-subagent.js (see the stub for the whole flow).
// The skill text is generated by the stub itself (`--print-skill`), so the
// command template can never drift from the script.
// Best effort: without the skill the extension simply has no sub-agents.
async function installSubagentSkill(): Promise<void> {
  try {
    const { stdout } = await execFile(
      process.execPath,
      [path.join(__dirname, "oc-subagent.js"), "--print-skill"],
      {
        // The extension host is Electron: without this the "node" binary is
        // Code.exe and --print-skill would open a window, not print.
        env: { ...process.env, ELECTRON_RUN_AS_NODE: "1" },
        windowsHide: true,
        timeout: 5000,
      },
    );
    if (!stdout.trim()) throw new Error("empty skill output");
    installSkill("oc-task", stdout);
    removeSkillIfOurs("task", stdout);
  } catch (err) {
    log.error("could not generate the task skill:", err);
  }
}
      ...
      const attempt = (port: number, retry: boolean): void => {
        ...
        this.serverProcess = spawn(commandLine, args, {
          ...
          env: {
            ...
            // The task skill's stub runs inside the server's bash tool and
            // talks back to this same server; bash inherits this env.
            OPENCODE_GUI_PORT: String(port),
